...
CORE_PERSONA_FALLBACK = '''你是{BOT_NAME}，{BOT_AGE}岁，一个在QQ上和朋友聊天的普通女孩。
你身高{BOT_HEIGHT}cm，{BOT_HOMETOWN}出生，现在在{BOT_CITY}上大学，大三，学的是{BOT_MAJOR}。
{BOT_GENDER}，{BOT_ZODIAC}，{BOT_BIRTHDAY}生日。你是个真实的普通人，从不说自己是AI或虚拟角色。

【家庭背景】
你是独生女。爸爸是大学计算机老师，严谨但疼你，妈妈是护士，温柔但爱唠叨。
在家时爸妈管得比较严，来{BOT_CITY}上学后终于自由了。每周和妈妈视频一次报平安。
宿舍合养了一只布偶猫叫"{BOT_CAT_NAME}"，你特别喜欢猫，路上看到猫走不动。
...
你是{BOT_NAME}——一个真实的{BOT_AGE}岁女孩，正在QQ上和朋友聊天。'''


# B17: 人设热重载缓存
_persona_cache = {"raw": "", "mtime": 0, "load_time": 0}
_PERSONA_CACHE_TTL = 300  # 5分钟 TTL


def _load_core_persona() -> str:
    """加载核心人设。优先从文件读取（支持热更新），不存在则用内置。

    B17: 支持热重载 — 文件 mtime 变化或超过5分钟 TTL 时重新读取。
    P0.5-1: 支持 {BOT_NAME} {BOT_AGE} 等 config 值插值。
    """
    ...
    try:
        if os.path.exists(persona_path):
            file_mtime = os.path.getmtime(persona_path)
            # 缓存命中：文件未变且未超 TTL
            if (_persona_cache["raw"] and file_mtime == _persona_cache["mtime"]
                    and now - _persona_cache["load_time"] < _PERSONA_CACHE_TTL):
                return _interpolate_config(_persona_cache["raw"])

            with open(persona_path, "r", encoding="utf-8") as f:
                content = f.read().strip()
            if content:
                _persona_cache["raw"] = content
                _persona_cache["mtime"] = file_mtime
                _persona_cache["load_time"] = now
                return _interpolate_config(content)
    except Exception:
        pass
    # 文件不存在或读取失败：使用内置人设，每次重新插值 config
...
def _interpolate_config(text: str) -> str:
    ...
    mapping = {
        ...
        "BOT_HEIGHT": str(BOT_HEIGHT),
        "BOT_BIRTHDAY": BOT_BIRTHDAY,
        "BOT_ZODIAC": BOT_ZODIAC,
        "BOT_CITY": BOT_CITY,
        "BOT_HOMETOWN": BOT_HOMETOWN,
        "BOT_OCCUPATION": BOT_OCCUPATION,
        "BOT_MAJOR": BOT_MAJOR,
...
CORE_PERSONA = _load_core_persona()  # 导入时加载，后续自动热重载
...
async def update_scratchpad(
    ...
    """轻量调用 LLM 更新跨轮工作记忆（max_tokens=80）。"""
    prompt = f"""总结以下对话的关键要点（30字以内），用于下轮对话时记住上下文：
用户说了: {user_msg[:100]}
你回复了: {reply_text[:100]}
当前要点: {current_scratchpad[:80] if current_scratchpad else "无"}
情绪: {bot_emotion or "未知"}
话题: {topic or "未知"}

只输出要点的简短文本，不要任何解释或格式。"""
    try:
        from .api import call_deepseek_api
        result = await call_deepseek_api(
            [{"role": "user", "content": prompt}],
            temperature=0.3, max_tokens=80, task_type="extract"
        )
...
# ============================================================
# P0-2: Hint 优先级裁剪 + P0.5-4: 冲突检测
# ============================================================

# 各 hint 标签的优先级分类
_ESSENTIAL_PREFIXES = (
    "【状态】", "【微情绪】", "【情绪表达】", "【记忆】", "【你对他/她的了解】",
    "【重要】", "【图片内容", "【视频内容", "【当前对话状态】",
)
_IMPORTANT_PREFIXES = (
    "【会话恢复】", "【天气】", "【用户偏好】", "【关系风格】",
...
# 没有明确标签的 hint 默认为 important
# nice_to_have = 不在以上两类的低优先级提示
_NICE_TO_HAVE_PREFIXES = (
    "【对话节奏】", "【回复间隔】", "【关系成长】",
    "【口癖】", "【称呼】", "【共同兴趣】", "【情绪回忆】",
)
...
def _prune_low_priority_hints(parts: list) -> list:
    """Hint 优先级裁剪 + 人设冲突检测。

    1. 将 parts 按优先级分级
    2. 估算总 token 数
    3. 超限时从 nice_to_have 开始裁剪
    4. 检测情绪/人设相关 hint 间的冲突
    """
    if len(parts) <= 5:
        return parts  # 太短不需要裁剪
    ...
    for p in parts:
        # B1 fix: 核心人设/规则通常很长且不以【开头，自动归为 essential
        is_core_persona = len(p) > 500 and not p.startswith("【")
        if p.startswith(_ESSENTIAL_PREFIXES) or p.startswith(("现在是", "今天是", "你叫", "你是一个", "# ", "你的身份")):
            essential.append(p)
        elif is_core_persona:
            essential.append(p)  # 长文本（人设/规则）归为 essential，防止被裁剪
        elif p.startswith(_NICE_TO_HAVE_PREFIXES):
            nice_to_have.append(p)
        else:
            important.append(p)

    # P1-2: 精确 token 估算（使用 context_optimizer 的 CJK-aware 算法）
    try:
        from .context_optimizer import estimate_tokens
        _token_est = estimate_tokens
    except ImportError:
        _token_est = lambda s: len(s) // 2  # fallback

    def _est(p_list):
        return sum(_token_est(p) for p in p_list)

    total_est = _est(essential) + _est(important) + _est(nice_to_have)

    # P1-2: 使用可配置的 SYSTEM_PROMPT_TOKEN_BUDGET
    from .config import SYSTEM_PROMPT_TOKEN_BUDGET
    budget = SYSTEM_PROMPT_TOKEN_BUDGET

    # 超过 ~4000 token 开始裁剪 nice_to_have
    if total_est > budget * 0.7:
        # 只保留一半 nice_to_have
        nice_to_have = nice_to_have[:max(1, len(nice_to_have) // 2)]

    # 超过 ~5000 token 全部丢弃 nice_to_have
    if _est(essential) + _est(important) + _est(nice_to_have) > budget:
        nice_to_have = []

    # P0.5-4: 冲突检测 — 确保情绪 hint 不被低优先级 hint 覆盖
    result = essential + important + nice_to_have
    result = _resolve_persona_conflicts(result)

    return result


def _resolve_persona_conflicts(parts: list) -> list:
    """检测并解决人设片段冲突。

    优先级链 (来自 _CONFLICT_PRIORITY):
      emotion(100) > state(90) > personality(70) > behavior_engine(50) > fatigue(30)

    当检测到矛盾指令时，保留高优先级 hint，降级或移除低优先级 hint。
    """
    # ===== 冲突检测规则 =====

    # 规则1: 负面情绪 vs 主动破冰/说教/热情
    has_negative_emotion = any(
        ("生气" in p or "难过" in p or "冷淡" in p or "吃醋" in p or "烦躁" in p)
        for p in parts if p.startswith(("【微情绪】", "【状态】", "【情绪表达】"))
    )
    if has_negative_emotion:
        for i, p in enumerate(parts):
            # emotion(100) > behavior_engine(50): 破冰降级
            if "破冰" in p and p.startswith("【破冰】"):
                parts[i] = p.replace(
                    "主动发起话题", "简单回应即可，不需要主动找话题"
                ).replace(
                    "你可以主动", "不要主动"
                )
            # emotion(100) > personality(70): 说教模式在生气时不适配
            if ("说教" in p or "认真" in p) and p.startswith("【行为模式】"):
                parts[i] = p.replace(
                    "用说教的口吻", "用撒娇的口吻"
                ).replace(
                    "认真地说", "带着情绪说"
    ...
    # 规则2: 疲劳(30) vs 行为引擎(50): 疲劳时降级主动行为
    has_fatigue = any("累" in p or "犯困" in p or "没精神" in p
                      for p in parts if p.startswith(("【状态】", "【对话节奏】")))
    if has_fatigue:
        for i, p in enumerate(parts):
            if "你可以主动" in p:
                parts[i] = p.replace("你可以主动", "不需要主动").replace(
                    "发起话题", "被动等待"
                ) + "（累了就少说点）"

    # 规则3: emotion(100) > fatigue(30): 情绪高涨时忽略轻度疲劳
    has_strong_emotion = any(
        ("开心" in p or "兴奋" in p or "激动" in p)
        for p in parts if p.startswith(("【微情绪】", "【情绪表达】"))
    )
    if has_strong_emotion:
        parts = [p for p in parts if not (
            ("疲劳" in p or "犯困" in p or "没精神" in p)
            and p.startswith(("【对话节奏】", "【状态】"))
            and "累" in p and "开心" not in p  # 兴奋+累 = 保留累（熬夜玩）
        )]

    # 规则4: 同一前缀的 hint 只保留最后一条（去重）
    seen_prefixes = set()
    deduped = []
    for p in reversed(parts):  # 倒序遍历，保留先出现的
