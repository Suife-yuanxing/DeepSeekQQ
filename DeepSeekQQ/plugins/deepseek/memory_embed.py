...
P2-5 优化：
- NumPy 加速余弦相似度批量计算
- 批量 embedding API（单次请求多个 input）
- 部分索引（v16 migration，WHERE embedding IS NOT NULL）
"""
...
_EMBED_DIM = 1024
_EMBED_BATCH_MAX = 8  # P2-5: 批量 API 单次最多 8 条
...
async def get_embeddings_batch(texts: List[str]) -> List[Optional[List[float]]]:
    """P2-5: 批量获取 embedding（利用 GLM API 的批量 input 能力）。

    GLM embedding API 支持 `input` 为字符串数组，单次请求返回多个向量。
    """
    if not texts:
        return []
    ...
    for i, t in enumerate(texts):
        if not t or not t.strip():
            results[i] = None
        elif t.strip() in _embed_cache:
            results[i] = _embed_cache[t.strip()]
        else:
            uncached.append(t.strip())
            uncached_idx.append(i)

    if not uncached:
        return results

    # 分批请求（每批最多 _EMBED_BATCH_MAX 条）
    for batch_start in range(0, len(uncached), _EMBED_BATCH_MAX):
        batch = uncached[batch_start:batch_start + _EMBED_BATCH_MAX]
        batch_indices = uncached_idx[batch_start:batch_start + _EMBED_BATCH_MAX]

        try:
            from .api import get_http_session
            session = await get_http_session()
            payload = {"model": _EMBED_MODEL, "input": batch}
            headers = {
                "Authorization": f"Bearer {GLM_API_KEY}",
                "Content-Type": "application/json",
            }
            async with session.post(
                ...
                if resp.status != 200:
                    logger.warning(f"[Embed] 批量 API 错误 {resp.status}")
                    continue
                data = await resp.json()
                embeddings = data.get("data", [])
                for j, emb_item in enumerate(embeddings):
                    if j < len(batch_indices):
                        idx = batch_indices[j]
                        emb = emb_item.get("embedding")
                        if emb and len(emb) == _EMBED_DIM:
                            _add_to_cache(batch[j], emb)
                            results[idx] = emb
        except asyncio.TimeoutError:
            logger.warning("[Embed] 批量 API 超时")
        except Exception as e:
            logger.warning(f"[Embed] 批量 API 异常: {e}")
...
def cosine_similarity_batch(query: List[float], candidates: List[List[float]]) -> List[float]:
    """P2-5: NumPy 加速批量余弦相似度计算。

    Args:
        query: 查询向量 [D]
        candidates: 候选向量列表 N×[D]

    Returns:
        相似度列表 [N]，与 candidates 一一对应
    """
    if not candidates:
        return []
    ...
    if _HAS_NUMPY:
        ...
        candidates_np = np.array(candidates, dtype=np.float32)  # (N, D)

        dot = np.dot(candidates_np, query_np)  # (N,)
        norm_q = np.linalg.norm(query_np)
        norm_c = np.linalg.norm(candidates_np, axis=1)  # (N,)
...
async def semantic_search_memories(
    ...
    for row in rows:
        ...
        candidate_embs.append(tag_emb)

    if not candidate_embs:
        return []

    # 批量计算余弦相似度
    sims = cosine_similarity_batch(query_emb, candidate_embs)

    results = []
    for tag_id, sim in zip(tag_ids, sims):
        if sim > 0.3:  # 最小相似度阈值
