...
@migration(16)
async def migrate_v16_add_performance_indexes(db: aiosqlite.Connection):
    """P1-8: 添加6个性能索引（含部分索引），优化查询性能。

    索引列表：
    1. memory_tags: 部分索引 WHERE embedding IS NOT NULL（语义搜索加速）
    2. memory_tags: (user_id, last_used) — 衰减查询
    3. user_preferences: (user_id, last_updated) — 偏好衰减
    4. shared_memories: (user_id, last_recalled) — 回忆重新度
    5. proactive_log: (user_id, created_at) — 去重查询
    6. affection: (user_id, last_interaction) — 衰减调度
    """
    indexes = [
        # 1. 部分索引：只索引有 embedding 的记录，加速语义搜索
        "CREATE INDEX IF NOT EXISTS idx_memory_tags_embed ON memory_tags(user_id) WHERE embedding IS NOT NULL",
        # 2. 加速记忆衰减查询（按用户+上次使用时间排序）
        "CREATE INDEX IF NOT EXISTS idx_memory_tags_last_used ON memory_tags(user_id, last_used)",
        # 3. 加速偏好衰减查询
        "CREATE INDEX IF NOT EXISTS idx_user_pref_updated ON user_preferences(user_id, last_updated)",
        # 4. 加速共同回忆的重新度查询
        "CREATE INDEX IF NOT EXISTS idx_shared_mem_last_recalled ON shared_memories(user_id, last_recalled)",
        # 5. 加速主动消息去重查询
        "CREATE INDEX IF NOT EXISTS idx_proactive_created ON proactive_log(user_id, created_at)",
        # 6. 加速好感度衰减调度
        "CREATE INDEX IF NOT EXISTS idx_affection_interaction ON affection(user_id, last_interaction)",
    ]
    for sql in indexes:
        try:
