...
| 🎭 **真人化** | 早晚安模板(90%省LLM)/活动感知回复/can_interrupt/亲密过滤/情绪隐藏/非语言信号→情绪反馈 |
| 🧬 **因果上下文** | CausalContext 会话级共享状态，因果链追踪，活动→回复速度→情绪→早晚安全链路 |
| 📊 **行为引擎** | 7层优先级链(天气>季节>节日>刷屏>热搜>微事件>随机)，概率性单选替代随机合并 |
| 🫥 **缺席系统** | 6种真人缺席模型(上课/游戏/午睡/做饭/没电/通勤)，缺席时延迟/跳过回复，恢复后自然解释 |
...
共 1303 个测试（69 个测试文件），包含单元测试和集成测试。真人化改造专项测试覆盖因果链追踪/情绪累积/非语言信号/缺席事件/参数调优等全部 5 阶段功能。
...
Pipeline 阶段：handler.py（99行入口） + stages/ 目录（22个独立阶段文件）
...
- **2026-06-19**: 真人化 Phase 0 完成 — 早安亲密过滤+好感排序、早晚安35+20模板(90%/10%LLM)、skips持久化DB、prompt规则一致性修复、周评估默认开启+过滤、can_interrupt接入pipeline
