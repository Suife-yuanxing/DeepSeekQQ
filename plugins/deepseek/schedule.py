...
def get_schedule_state(hour: int = None, weekday: int = None) -> ScheduleState:
    ...
    if offsets["late_night"] and 0 <= hour < 2:
        return ScheduleState("night_owl", 0.3, 0.6, "normal",
            "深夜了还不困，在刷手机追番，偶尔回消息")
    ...
    # 起床后的间隙（wake_int+1 到 9点）：仍视为迷糊状态
    if wake_int + 1 <= hour < 9:
        if is_weekend and offsets["weekend_late"]:
            return ScheduleState("waking", 0.4, 0.65, "minimal",
                "周末赖床中，半梦半醒地刷手机")
        return ScheduleState("waking", 0.4, 0.65, "minimal",
            "刚起床没多久，还有点迷糊")
    ...
    if not is_weekend and offsets["skip_class"] and 9 <= hour < 12:
        return ScheduleState("lazy", 0.5, 0.7, "normal",
            "今天不想上课，躲在宿舍里刷手机打发时间")
    ...
    # 阶段10: 晚间自由时间 19:00-23:00: 精力最好（自由时间！）
    if 19 <= hour < 23:
