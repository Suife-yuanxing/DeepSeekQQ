...
def _prune_low_priority_hints(parts: list) -> list:
    ...
    # 简单 token 估算（中文 ~1.5 字/token）
    def _est(p_list):
        return sum(len(p) // 2 for p in p_list)

    total_est = _est(essential) + _est(important) + _est(nice_to_have)

    # 超过 ~4000 token（28K 预算的安全边界）开始裁剪 nice_to_have
    if total_est > 4000:
        # 只保留一半 nice_to_have
        nice_to_have = nice_to_have[:max(1, len(nice_to_have) // 2)]

    # 超过 ~5000 token 全部丢弃 nice_to_have
    if _est(essential) + _est(important) + _est(nice_to_have) > 5000:
        nice_to_have = []
