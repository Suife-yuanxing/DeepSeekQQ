...
def _prune_low_priority_hints(parts: list) -> list:
    ...
    for p in parts:
        if p.startswith(_ESSENTIAL_PREFIXES) or p.startswith(("现在是", "你叫", "你是一个", "# ")):
            essential.append(p)
...
def _resolve_persona_conflicts(parts: list) -> list:
    ...
    优先级: emotion > personality > behavior_engine > fatigue
    当检测到矛盾指令时（如"你有点生气"同时"主动破冰"），
    保留高优先级 hint，给低优先级 hint 附加协调指令。
    """
    has_negative_emotion = any(
        ("生气" in p or "难过" in p or "冷淡" in p or "吃醋" in p)
        for p in parts if p.startswith(("【微情绪】", "【状态】"))
    )
    has_icebreaker = any("破冰" in p for p in parts)

    if has_negative_emotion and has_icebreaker:
        # 负面情绪时冰breaker降级：被动回应而非主动找话题
